...
function addTopicList(topicListName, easyWordNames, mediumWordNames, hardWordNames) {
  var newTopicList = [];
  ...
  topicLists[topicListName] = newTopicList;
}
...
module.exports = function (io, socket) {
  ...
  // TODO: but since words = [ {word: '___', difficulty: '___'}] it might not work
  var topicListWordNames = getTopicListWordNames('default', 'all');

  function sendTopic() {
    // Select a new topic and send it to the new drawer
    topicListWordNames.push(topicListWordNames.shift());
    Game.getDrawers().forEach(function (drawer) {
      io.to(drawer).emit('topic', topicListWordNames[0]);
    });
  ...
  function advanceRound() {
    ...
    broadcastMessage({
      class: 'status',
      text: 'Round over! The topic was "' + topicListWordNames[0] + '"'
    });
  ...
  socket.on('requestState', function () {
    ...
    if (Game.isDrawer(username)) {
      socket.emit('topic', topicListWordNames[0]);
    }
  ...
  socket.on('gameMessage', function (message) {
    ...
    var topic = topicListWordNames[0].toLowerCase();
